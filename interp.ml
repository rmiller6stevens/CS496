...
and
  ...
  | Proj(e,id) ->
    eval_expr e >>=
    fields_of_recordVal >>= fun recx ->
    (match (List.assoc_opt id recx) with
    | Some (true, RefVal l) -> (Store.deref g_store l >>= fun vals -> return vals)
    | Some (false, ev) -> return ev
    | _ -> error "Field not found.")
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=
    fields_of_recordVal >>= fun recx ->
    eval_expr e2 >>= fun newV ->
    (match List.assoc_opt id recx with
    | Some (false, ev) -> error "Field not mutable"
    | Some (true, RefVal l) -> 
      (Store.set_ref g_store l newV >>= fun dontuse -> return UnitVal) 
    | _ -> error "Field not found.")
  | IsNumber(e) ->
    eval_expr e >>= fun ans ->
    (match ans with
